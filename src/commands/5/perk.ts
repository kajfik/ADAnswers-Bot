...
export const perk: Command = {
  ...
  run: async(interaction: ChatInputCommandInteraction) => {
    ...
    if (interaction.options.data.length > 1) {
      await interaction.reply({ content: "You can only use one perk type at a time.", flags: MessageFlags.Ephemeral });
      return;
    }

    if (interaction.options.data.length === 0) {
      await interaction.reply({ content: "You must specify a perk type.", flags: MessageFlags.Ephemeral });
      return;
    }
