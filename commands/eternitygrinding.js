...
module.exports = {
  ...
  execute(message, args, id) {
    if (functions.special.eternityGrindingCheck(id, message)) {
      switch (args[0]) {
      case "early":
...
Test which works the best for you. You might have to adjust the value a little bit. Use ID+active path.`);
        break;
      case "late":
        message.channel.send(`ETERNITY FARMING: (Do this on long AFKs up to 1M)
        ...
        break;
      default: 
        message.channel.send(functions.getMessage("error", { args, name: this.name, acceptableArgs: ["early", "late"] }));
      }
