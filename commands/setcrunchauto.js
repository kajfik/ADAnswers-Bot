...
module.exports = {
  ...
  execute(message, _a, id) {
    if (config.ids.earlyGame.includes(id) || config.ids.common.includes(id) || config.ids.botCommands.includes(id)) message.channel.send(`Look at how much IP you would gain on crunch when you reach your peak IP/min, and set it to that. It will probably be about a second after you get your last galaxy (make sure your dimboosts are off, unless you have the bulk dimboost upgrade)`);
